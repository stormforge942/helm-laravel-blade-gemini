...
class SiteGeneratorController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('creation.generate-site.index', compact('user', 'permissions'));
    }
    ...
    public function generate(Request $request)
    {
        try {
            ...
            // Return the URL to the generated site view
            return response()->json(['redirect_url' => route('generated.site.view')]);
    ...
    private function generateContent($data)
    {
        if (stripos($data['keywords'], 'fencing') !== false || stripos($data['core_focus'], 'fencing') !== false) {
            ...
            $data['core_focus'] = str_ireplace('fencing', 'fencing (building a physical fence for homes)', $data['core_focus']);
        }
        ...
        if (!empty($data['content_style'])) {
            if (in_array('bullet-points', $data['content_style']) && in_array('numbering', $data['content_style'])) {
                $contentStyleInstructions = "Organize the content using both bullet points (<ul> and <li> tags) and numbering (<ol> and <li> tags) where applicable. Use <ol> for ordered lists. Use <ul> for unordered lists and <li> for list items.";
            } elseif (in_array('bullet-points', $data['content_style']) && !in_array('numbering', $data['content_style']) ) {
                $contentStyleInstructions = "Organize the content using bullet points (<ul> and <li> tags) where applicable. Use <ul> for unordered lists and <li> for list items. Do not use numbering.";
            } elseif (in_array('numbering', $data['content_style']) && !in_array('bullet-points', $data['content_style']) ) {
                $contentStyleInstructions = "Organize the content using numbering (<ol> and <li> tags) where applicable. Use <ol> for ordered lists. Do not use bullet points.";
            }
        }

        // $prompt = "Generate a unique version of the provided commercial services content that is highly informative, reliable, and useful for people in the specified locality. The content must be between {$data['min_word_count']} and {$data['max_word_count']} words. Adhere to the required tone, emphasize the given keywords, and ensure the content follows these guidelines:
        ...
        - **The content must be between {$data['min_word_count']} and {$data['max_word_count']} words. The word count requirement is strict and should be adhered to.**
    ...
    public function viewGeneratedSite(Request $request)
    {
